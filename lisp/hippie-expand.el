;;; hippie-expand.el --- expand text trying various ways to find its expansion.
...
;;  6-August-95|Version 1.4|~/misc/hippie-expand.el.Z|
...
;; First, lets define some things that may not be present outside Emacs19
(if (not (fboundp 'member))
(defun member (ele lst)
  "Return non-nil if ELT is an element of LIST.  Comparison done with `equal'.
The value is actually the tail of LIST whose car is ELT."
  (while (and lst (not (equal (car lst) ele)))
    (setq lst (cdr lst)))
  lst)
)
(if (not (fboundp 'window-minibuffer-p))
(defun window-minibuffer-p (&optional win)
...
(if (not (fboundp 'skip-syntax-backward))
(defun skip-syntax-backward (str)
  "Move point backward across chars in specified syntax classes."
  (let ((cnt 0)
	(neg (= (aref str 0) ?^)))
    (while (and (not (bobp))
		(eq neg
		    (not (string-match (regexp-quote (char-to-string
						      (char-syntax
						       (preceding-char))))
				       str))))
      (backward-char)
...
(if (not (fboundp 'skip-syntax-forward))
(defun skip-syntax-forward (str)
...
;; Now we can go on just as usual
...
;;;###autoload
(defvar hippie-expand-load-hook () 
  "Run after loading `hippie-expand', for customization.
Here can for example `hippie-expand-try-functions-list' be changed,
and functions be defined with `make-hippie-expand-function'.")
...
string).  It returns t if a new expansion is found, nil otherwise."
  (let ((expansion ())
	...
	(set-marker he-search-loc (point) (current-buffer)))
      (if (not expansion)
	  (progn 
	    (setq he-search-window (next-window he-search-window))
...
(run-hooks 'hippie-expand-load-hook)
