...
(defun css-color-turn-on-in-buffer ()
  ...
  (when (derived-mode-p 'css-mode)
    (message "turn-on-in-b:before (css-color-mode 1) cb=%s" (current-buffer))
    (css-color-mode 1)
    (message "turn-on-in-b:after (css-color-mode 1)")
    ))
...
(defvar css-color-keywords
  `((,css-color-hex-re
    ...
    (,css-color-html-re
     (0
      (let ((color (cadr (assoc (match-string-no-properties 0) css-color-html-colors))))
	(put-text-property (match-beginning 0)
			   (match-end 0)
